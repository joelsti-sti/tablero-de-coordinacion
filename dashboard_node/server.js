...
// ---------------------------------------------------------------
// KPI Mensual - Reporte Cierre de Mes
// ---------------------------------------------------------------
app.get('/api/kpi-mensual', async (req, res, next) => {
    try {
        ...
        // 1. FS del mes por técnico con tipo, ausencias, horas
        const [fsRows] = await pool.query(`
            SELECT
                ...
                COALESCE(scf.cf_1211, 0) as viaje_ida,
                COALESCE(scf.cf_1213, 0) as viaje_vuelta,
                scf.cf_922 as tipo_servicio,
                scf.cf_930 as codigo_tipo,
                scf.cf_958 as abonado_tipo,
                CASE
                    WHEN scf.cf_1046 = 1 THEN 'Ausente con Justificación'
                    WHEN scf.cf_1048 = 1 THEN 'Ausente sin aviso'
                    WHEN scf.cf_1050 = 1 THEN 'Ausente autorizado'
        ...
        // 2. Relaciones FS -> Tickets y Tareas (para detectar no asociados)
        const [relRows] = await pool.query(`
            SELECT fs_id FROM (
                SELECT crmid as fs_id FROM vtiger_crmentityrel
                WHERE module = 'ServiceContracts' AND relmodule IN ('HelpDesk', 'ProjectTask')
        ...
        // 3. Tareas completadas en el mes por técnico
        const [tareasRows] = await pool.query(`
            SELECT
                cr.smownerid as user_id,
                CONCAT(u.first_name, ' ', u.last_name) as nombre,
                COUNT(*) as total,
        ...
        // 4. Tickets resueltos en el mes por técnico
        ...
        // 5. Tickets abiertos por prioridad y antiguedad
        const [ticketsAbiertos] = await pool.query(`
            SELECT
                ...
                CONCAT(u.first_name, ' ', u.last_name) as assigned_user,
                cr.createdtime,
                DATEDIFF(NOW(), cr.createdtime) as dias_abierto
            FROM vtiger_troubletickets tk
            JOIN vtiger_crmentity cr ON tk.ticketid = cr.crmid AND cr.deleted = 0
            LEFT JOIN vtiger_account a ON tk.parent_id = a.accountid
            LEFT JOIN vtiger_users u ON cr.smownerid = u.id
        ...
        // 6. Tareas pendientes cerradas sin solucion (todo el historial)
        ...
        // 7. Tickets pendientes sin solucion (mes actual)
        ...
        // 8. Proximos mantenimientos agendados (del mes siguiente)
        ...
        for (const fs of fsRows) {
            ...
            t.fs_list.push({
                fs_id: fs.fs_id,
                contract_no: fs.contract_no,
                subject: fs.subject,
                cliente: fs.cliente || 'STI',
                fecha: fs.fecha,
                hora_inicio: fs.hora_inicio,
                hora_fin: fs.hora_fin,
                horas: fs.horas,
                modalidad: fs.modalidad,
                tipo_servicio: fs.tipo_servicio,
                sin_asociar: sinTicket
            });
        }

        // Merge tareas completadas
        const tareasMap = new Map();
        for (const r of tareasRows) tareasMap.set(r.user_id, r);

        // Merge tickets resueltos
        ...
        for (const [, t] of tecMap) {
            ...
            tecnicos.push({
                ...t,
                dias_trabajados: t.dias_trabajados.size,
        ...
        // Mantenimientos sin asociar (pendientes)
        const mantenimientosSinAsociar = fsRows.filter(fs =>
            ...
            modalidad: fs.modalidad,
            horas: fs.horas,
            motivo: 'Sin ticket ni tarea asociada'
        }));

        // Ticket KPIs
        const urgentes24h = ticketsAbiertos.filter(t =>
            t.priority === 'Urgent' && t.dias_abierto >= 1
        );
        const alta7d = ticketsAbiertos.filter(t =>
            t.priority === 'High' && t.dias_abierto >= 7
        );
        const baja30d = ticketsAbiertos.filter(t =>
            (t.priority === 'Low' || t.priority === '') && t.dias_abierto >= 30
        );
        const totales_abiertos = ticketsAbiertos.length;

        const respuesta = {
            periodo: { year: parseInt(year), month: parseInt(month) },
            mes_nombre: new Date(year, month - 1, 1).toLocaleString('es', { month: 'long', year: 'numeric' }),
            tecnicos,
